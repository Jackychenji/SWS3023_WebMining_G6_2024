...
def create_binary_list_artist(artists_list, prefer_artists_name):
    prefer_artists_name = [decade.lower() for decade in prefer_artists_name]

    binary_list = [0] * len(artists_list)  # Initialize a list of 19 zeros

    # Check each genre tag against the `prefer_genre` list and set the corresponding index to 1 if present
    for index, artist in enumerate(artists_list):
        if artist in prefer_artists_name:
            binary_list[index] = 1

    return binary_list


# artists_str是第三题的全部20歌歌手的str,prefer_artists_name是选中的x/20个歌手(顺序无所谓)，artist_prefer是返回的20列01数组
def data_process(bigTag_prefer, subTag_prefer, decade_prefer, artists_str, prefer_artists_name):
    ...
    filtered_album_df = album_sub_tag_df[album_sub_tag_df['artist'].isin(prefer_artists_name)]
    artists_list = [artist.strip() for artist in artists_str.split(', ')]

    for artist in artists_list:
        sub_tag_df[artist] = 0

    for index, row in filtered_album_df.iterrows():
        album_name = row['name']
        artist = row['artist']
        if artist in artists_list and album_name in sub_tag_df.index:
            sub_tag_df.at[album_name, artist] = 1
    artist_prefer = create_binary_list_artist(artists_list, prefer_artists_name)
    decade_prefer = create_binary_list_decade(decade_prefer)
    input_str = subTag_prefer + decade_prefer + artist_prefer
